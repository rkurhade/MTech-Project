...
class AppController:
    ...
    def send_upcoming_expiry_notifications(self, days=30, resend_interval=3):
        ...
        Resends every `resend_interval` days if not rotated.
        """
        ...
        for user_name, email, app_name, expires_on, last_notified_at in expiring_secrets:
            try:
                # Resend only if never notified OR last notification > resend_interval
                if last_notified_at is None or (now - last_notified_at).days >= resend_interval:
                    print(f"[DEBUG] Sending upcoming expiry email: {app_name} - {expires_on}")
                    ...
                    # Mark as notified and update last_notified_at
                    self.user_service.mark_as_notified(app_name, column="notified_upcoming", update_last_notified=True)
