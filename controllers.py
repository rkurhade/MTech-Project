...
class AppController:
    ...
    def create_application(self, user_name, email, app_name):
        ...
        if is_testing:
            ...
            expires_on = datetime.now(ist) + timedelta(minutes=1)
        else:
            print("[INFO] EXPIRY_TEST_MODE is OFF: Using 24-month expiry.")
            expires_on = datetime.now(ist) + timedelta(days=730)
    ...
    def send_upcoming_expiry_notifications(self, days=3):
        ...
        for user_name, email, app_name, expires_on in expiring_secrets:
            try:
                print(f"[DEBUG] Expiring soon: {app_name} - {expires_on}")

                email_body_html = f"""
                <html>
                  <body style="font-family: Arial, sans-serif; color: #333;">
                    <p>Hi {user_name},</p>
                    <p><strong>Heads up:</strong> Your Azure Service Principal secret for <strong>{app_name}</strong> will expire on <strong>{expires_on.strftime('%Y-%m-%d %H:%M:%S')}</strong>.</p>
                ...
                print(f"[DEBUG] Marking as notified: {app_name}")
        ...
        for user_name, email, app_name, expires_on in expired_secrets:
            try:
                ...
                <html>
                    <body style="font-family: Arial, sans-serif; color: #333;">
                        <p>Hi {user_name},</p>
                        <p><strong>Action Required:</strong> Your Azure Service Principal secret for <strong>{app_name}</strong> expired on <strong>{expires_on.strftime('%Y-%m-%d %H:%M:%S')}</strong>.</p>
